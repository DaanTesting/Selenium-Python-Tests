...
@pytest.fixture(params=ChargingSessionData.refresh_adhoc_cp_data)
...
class TestThree(BaseClass):
    def test_set_pricing_policy(self, login_data, adhoc_data):
        log = self.get_logger()
        log.info(login_data["account"])
        ...
        locationsmainpage.find_location().send_keys(adhoc_data["AdhocLocation"] + Keys.ENTER
        )
        self.driver.find_element(By.XPATH, "//a[.='Autotesting Adhoc Location']").click()
        ...
        individualcharginglocation.choose_adhoc_policy()
...
class TestFour(BaseClass):
    def test_adhoc_payment_page_pricing(self):
        ...
        adhocwebapp.get_staging_adhoc_web_app()
        time.sleep(1)
