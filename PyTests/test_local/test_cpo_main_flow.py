...
class TestOne(BaseClass):
    def test_create_user(self, setup):
        log = self.get_logger()
        log.info("daan.swinnen+autotest_user@optimile.eu")
        log.info("Attempting login.")

        loginpage = LoginPage(self.driver)

        loginpage.username_box().send_keys(
            "daan.swinnen+autotest_user@optimile.eu"
        )
        loginpage.password_box().send_keys("test")
...
class TestTwo(BaseClass):
    def test_create_user_contract(self, setup, login_data):
        ...
        log.info("Open 'Automated Test Company Main'.")
        ...
        mspindividualcustomer.create_contract_select_user().select_by_index(1)
