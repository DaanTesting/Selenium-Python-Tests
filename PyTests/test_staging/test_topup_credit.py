...
class TestTwo(BaseClass):
    def test_topup_credit_stripe(self, setup, login_data):
        log = self.get_logger()
        log.info("Attempting login.")
        self.driver.get("https://test2.optimile.eu/")

        loginpage = LoginPage(self.driver)
        loginpage.username_box().send_keys("daan.swinnen+stripe_auto@optimile.eu")
        loginpage.password_box().send_keys("Auto14723269_")
        homepage = loginpage.login_button()
        log.info("Succesfully logged in.")
        log.info("Navigating to finance tab.")
        homepage.menu_label_finance()
